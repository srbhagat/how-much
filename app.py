...
CACHE_FILE = os.path.join(DATA_DIR, "coindcx_btcusdt_side_points_full_year.csv")
...
def build_or_update_database(years_requested=1, force=False):
    ...
    years_requested = 1
    target_start = pd.Timestamp.now(tz="UTC") - pd.DateOffset(years=1)
    ...
    if not existing.empty:
        existing = existing[(existing["open_time"] >= target_start_ms) & (existing["open_time"] <= now_ms)].copy()
    if not existing.empty and len(existing) < expected * 0.8:
    ...
    while cursor < now_ms:
        ...
        if frames:
            ...
            temp = temp[(temp["open_time"] >= target_start_ms) & (temp["open_time"] <= now_ms)].copy()
            candle_count = len(temp.drop_duplicates("open_time"))
        else:
            candle_count = 0

        progress.progress(
            min(1.0, max(0.0, (cursor - target_start_ms) / total_span)),
            text=f"CoinDCX forward history: batch {batch}, candles {candle_count:,}, empty windows {empty_windows}"
        )
    ...
    if len(df) < expected * 0.8:
        st.warning(f"CoinDCX returned only {len(df):,} candles out of expected {expected:,}. History is incomplete for this pair/timeframe.")
