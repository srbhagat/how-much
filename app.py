...
CACHE_FILE = os.path.join(DATA_DIR, "coindcx_btcusdt_side_points_root_fixed.csv")

STEP_MS = 15 * 60 * 1000

def clean_datetime_column(df):
    """
    Root fix for Force Rebuild crash:
    Convert datetime safely, drop invalid rows, and keep one consistent timezone.
    """
...
st.title("How Much Possible Which Side - Root Fixed")
st.caption("Root-fixed version: safe datetime cleaning + CoinDCX data + possible TP/SL calculator.")
